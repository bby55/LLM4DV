...
class CoverageMonitor:
    ...
    def sample_coverage(self):
        if to_int(self.signals["valid"].value):
            self.last_values.append(int(self.signals["value"].value))

            if len(self.last_values) > 16:
                self.last_values = self.last_values[-16:]

        if to_int(self.signals["stride_1_valid"].value):
            if to_int(self.signals["stride_2_valid"].value):
                self.coverage_database.stride_2_seen[to_int(self.signals["stride_1"].value)][
                    to_int(self.signals["stride_2"].value)
                ] += 1
            else:
                self.coverage_database.stride_1_seen[
                    to_int(self.signals["stride_1"].value)
                ] += 1
...
class SimulationController:
    ...
    def sample_dut_state(self):
        return DUTState(
            last_value=to_int(self.dut.last_value.value),
            stride_1=to_int(self.dut.stride_1_q.value),
            stride_1_confidence=to_int(self.dut.stride_1_confidence_q.value),
            stride_2=to_int_list(self.dut.stride_2_q.value),
            stride_2_state=to_int(self.dut.stride_2_state_q.value),
            stride_2_confidence=[
                to_int(self.dut.stride_2_confidence_q[0].value),
                to_int(self.dut.stride_2_confidence_q[1].value),
            ],
        )
...
async def basic_test(dut):
    ...
    while True:
        ...
        cocotb.start_soon(Clock(dut.clk_i, 10, unit="step").start())
        await do_reset(dut)
