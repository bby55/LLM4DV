...
class CoverageMonitor:
    ...
    def sample_alu_ops(self):
        ...
        if (
            ival(self.signals["rf_we"]) != 0
            and ival(self.signals["mult_sel"]) == 0
            and ival(self.signals["div_sel"]) == 0
            and ival(self.signals["rf_ren_a"]) != 0
            and ival(self.signals["alu_op_a_mux_sel"]) == ibex_consts.OP_A_REG_A
            and ival(self.signals["rf_wdata_sel"]) == ibex_consts.RF_WD_EX
        ):
            alu_op_str = self.alu_op_str_from_val(ival(self.signals["alu_operator"]))
            if alu_op_str:
                if ival(self.signals["alu_op_b_mux_sel"]) == ibex_consts.OP_B_IMM:
                    self.alu_imm_op_seen = alu_op_str
                else:
                    self.alu_op_seen = alu_op_str

    def sample_mem_ops(self):
        if ival(self.signals["data_req"]) != 0:
            access_size_str = self.access_size_str_from_type(
                ival(self.signals["data_type"])
            )

            if ival(self.signals["data_we"]) != 0:
                self.store_seen = access_size_str
            else:
                self.load_seen = access_size_str

    def sample_rf_accesses(self):
        if (ival(self.signals["rf_we"]) != 0) or (self.load_seen is not None):
            self.write_reg_seen = ival(self.signals["rf_waddr"])

        if ival(self.signals["rf_ren_a"]) != 0:
            self.read_reg_a_seen = ival(self.signals["rf_raddr_a"])

        if ival(self.signals["rf_ren_b"]) != 0:
            self.read_reg_b_seen = ival(self.signals["rf_raddr_b"])

    def sample_coverage(self):
        illegal_insn = False
        self.clear_seen()

        if ival(self.signals["illegal_insn"]) == 0:
            self.sample_alu_ops()
            self.sample_mem_ops()
...
async def basic_test(dut):
    ...
    server_port = os.environ.get("LLM4DV_PORT") or input("Please enter server's port (e.g. 5050, 5555): ")
